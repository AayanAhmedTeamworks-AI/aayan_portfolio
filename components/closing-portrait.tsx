...
/**
 * Closing portrait for Vita. Full-viewport image fades up with a slow scale
 * settle; on first entry, dispatches a `spotlight:bloom` CustomEvent that the
 * global <Spotlight/> listens for and brightens against — the page closes the
 * way a museum closes for the night, with a curator turning on the spot.
 */
export function ClosingPortrait({
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("spotlight:bloom", {
        detail: { duration: 900, scale: 1.55, intensity: 1.6 },
      }),
    );
  }, [inView, reduce]);
